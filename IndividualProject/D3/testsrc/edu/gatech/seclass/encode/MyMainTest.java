...
public class MyMainTest {
    ...
    */
    ...
    private static final String FILE3 = "abc123  !@# \n" +
        "xyz987 *()\n" +
        "def456";
    private static final String FILE4 = "abc123  !@# \n\n" +
            "xyz987 *()\n\r" +
            "def456";
    ...
    */
    ...
    // Frame #: Test Case 7
    // Failure Type: BUG Allows for an input outside of the acceptable range
    @Test
    ...
    // Purpose: Testing -d flag for input beyond allowable value
    // Failure Type: BUG Allows for an input outside of the acceptable range
    @Test
    public void encodeTest31() throws Exception {
        File inputFile30 = createInputFile(FILE3);

        String args[] = {"-d","-1", inputFile30.getPath()};
        Main.main(args);
    ...
    // Purpose: Testing -d flag for large input, beyond the need for any deletions
    @Test
    public void encodeTest32() throws Exception {
        File inputFile32 = createInputFile(FILE3);

        String args[] = {"-d","50", inputFile32.getPath()};
        Main.main(args);

        String expected32 = "abc123  !@# \n" +
                "xyz987 *()\n" +
                "def456";

        String actual32 = getFileContent(inputFile32.getPath());

        assertEquals("The outputs differ!", expected32, actual32);

    }

    // Purpose: Testing -d flag for input beyond allowable value
    // Failure Type: BUG Allows for an input outside of the acceptable range, input should be integer but float is accepted
    @Test
    public void encodeTest33() throws Exception {
        File inputFile33 = createInputFile(FILE3);

        String args[] = {"-d","1.5", inputFile33.getPath()};
        Main.main(args);
    ...
    // Purpose: Testing -c flag for improper input
    // Failure Type: Corner Case Improper output err stream
    @Test
    public void encodeTest34() throws Exception {
        File inputFile34 = createInputFile(FILE3);

        String args[] = {"-c","1", inputFile34.getPath()};
        Main.main(args);
    ...
    // Purpose: Testing -r flag for improper input

    @Test
    public void encodeTest35() throws Exception {
        File inputFile35 = createInputFile(FILE3);

        String args[] = {"-r","a", inputFile35.getPath()};
        Main.main(args);
    ...
    // Purpose: Testing two of the same flags for -r input. Should ignore the first input
    // Failure Type: BUG Reads first -r input as a -n input, code should ignore first -r flag and use -r flag as input
    @Test
    public void encodeTest36() throws Exception {
        File inputFile36 = createInputFile(FILE3);

        String args[] = {"-r","1","-r","3", inputFile36.getPath()};
        Main.main(args);

        String expected36 = "@# abc123  !\n" +
                "*()xyz987 \n" +
                "456def";

        String actual36 = getFileContent(inputFile36.getPath());

        assertEquals("The outputs differ!", expected36, actual36);

    }

    // Purpose: Testing two of the same flags for -n input. Should ignore the first input
    // Failure Type: BUG Number format exception is thrown when only the second -n input should be accepted. Code is parsing for an input after n when none is given
    @Test
    public void encodeTest37() throws Exception {
        File inputFile37 = createInputFile(FILE3);

        String args[] = {"-n","2","-n", inputFile37.getPath()};
        Main.main(args);

        String expected37 = "010203123  !@# \n" +
                "242526987 *()\n" +
                "040506456";

        String actual37 = getFileContent(inputFile37.getPath());

        assertEquals("The outputs differ!", expected37, actual37);

    }

    // Purpose: Testing -c flag with blank input.
    // Failure Type: BUG gives ArrayOutOfBoundException when -c with an empty input should not manipulate the string.
    @Test
    public void encodeTest38() throws Exception {
        File inputFile38 = createInputFile(FILE3);

        String args[] = {"-c","", inputFile38.getPath()};
        Main.main(args);

        String expected38 = "abc123  !@# \n" +
                "xyz987 *()\n" +
                "def456";

        String actual38 = getFileContent(inputFile38.getPath());

        assertEquals("The outputs differ!", expected38, actual38);

    }

    // Purpose: Testing -c flag with same uppercase and lowercase letter. Once lower case is turned into upper case, it should not turn back into lowercase unless another lowercase letter is supplied in the input
    // Failure Type: BUG After letter is converted to uppercase it is then converted back to lowercase, when it should remain uppercase. The whole string should not be parsed again for the conversion
    @Test
    public void encodeTest39() throws Exception {
        File inputFile39 = createInputFile(FILE3);

        String args[] = {"-c", "aA", inputFile39.getPath()};
        Main.main(args);

        String expected39 = "Abc123  !@# \n" +
                "xyz987 *()\n" +
                "def456";

        String actual39 = getFileContent(inputFile39.getPath());

        assertEquals("The outputs differ!", expected39, actual39);

    }

    // Purpose: Testing -c and -r flags with a combination of line separators back to back
    // Failure Type: BUG Throws arithmetic excpetion when lines should be separated
    @Test
    public void encodeTest40() throws Exception {
        File inputFile40 = createInputFile(FILE4);

        String args[] = {"-r","1", inputFile40.getPath()};
        Main.main(args);

        String expected40 = " abc123  !@#\n\n" +
                ")xyz987 *(\n\r" +
                "6def45";

        String actual40 = getFileContent(inputFile40.getPath());

        assertEquals("The outputs differ!", expected40, actual40);
